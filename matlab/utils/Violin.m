classdef Violin < handle
    ...
    properties (Dependent=true)
        ...
        WhiskerData         % custom whisker data
        QuartileData        % Custom Quartile Data
    end
        ...
        function obj = Violin(data, pos, varargin)
            ...
            switch args.QuartileStyle
                case 'shadow'
                    ...
                    if isempty(args.QuartileData)
                        indices = h >= quartiles(1) & h <= quartiles(3);
                    else
                        indices = h >= args.QuartileData(1) & h <= args.QuartileData(2);
                    end
            ...
            switch args.HalfViolin
                ...
                case 'full'
                    obj.MeanPlot = plot(pos+[-1,1].*meanDensityWidth, ...
                        [meanValue, meanValue],'LineStyle','-', 'Color','k');
            end
            obj.MeanPlot.LineWidth = 1.5;
            ...
            if isempty(args.WhiskerData)
                if ~isempty(lowhisker) && ~isempty(hiwhisker)
                    obj.WhiskerPlot = plot([pos pos], [lowhisker hiwhisker],...
                        'Marker','none','LineStyle','-');
                end
            else
                obj.WhiskerPlot = plot([pos pos], [args.WhiskerData(1) args.WhiskerData(2)],...
                    'Marker','none','LineStyle','-', 'Color', 'k');
                hold on,
                obj.WhiskerPlot = plot(pos+[-1,1].*0.8, [args.WhiskerData(1) args.WhiskerData(1)],...
                    'Marker','none','LineStyle','-', 'Color', 'k');
                obj.WhiskerPlot = plot(pos+[-1,1].*0.8, [args.WhiskerData(2) args.WhiskerData(2)],...
                    'Marker','none','LineStyle','-', 'Color', 'k');
    ...
    methods (Access=private)
        function results = checkInputs(~, data, pos, varargin)
            ...
            p.addParameter('QuartileData', []);
            p.addParameter('WhiskerData', []);
